...
class Weapon:
    ...
    def _nearest_enemy_from(self, origin):
        nearest = None
        best = float("inf")
        for enemy in self.game.enemies:
            d = origin.distance_squared_to(enemy.pos)
            if d < best:
                best = d
                nearest = enemy
        return nearest
...
class PencilWand(Weapon):
    ...
    def update(self):
        ...
        if now - self.last_shot > fire_rate:
            ...
            nearest_enemy = self._nearest_enemy_from(self.owner.pos)
...
class BombExplosion(pygame.sprite.Sprite):
    ...
    def deal_damage(self):
        cred = self.score_owner or self.game.player
        for enemy in self.game.enemies:
...
class DroppedBomb(pygame.sprite.Sprite):
    def __init__(self, game, x, y, fuse_time, radius, damage, owner=None):
        super().__init__()
        self.game = game
        self.owner = owner
    ...
    def update(self):
        ...
        if pygame.time.get_ticks() - self.spawn_time > self.fuse_time:
            cred = self.owner or self.game.player
            exp = BombExplosion(
                self.game, self.pos.x, self.pos.y, self.radius, self.damage, score_owner=cred
            )
...
class OrbitingBook(pygame.sprite.Sprite):
    def __init__(self, game, owner, angle_offset, radius, speed, damage):
        super().__init__()
        self.game = game
        self.owner = owner
...
class TextbookOrbit(Weapon):
    ...
    def update_books(self):
        ...
        for i in range(count):
            ...
            book = OrbitingBook(self.game, self.owner, angle_offset, radius, speed, damage)
...
def _nearest_enemy_from(game, origin):
    nearest = None
    min_dist = float("inf")
    for enemy in game.enemies:
        d = origin.distance_to(enemy.pos)
        if d < min_dist:
            min_dist = d
            nearest = enemy
    return nearest
...
class USBBoomerangSprite(pygame.sprite.Sprite):
    def __init__(self, game, owner, direction, damage):
        super().__init__()
        self.game = game
        self.owner = owner
...
class StickyMine(pygame.sprite.Sprite):
    def __init__(self, game, owner, x, y, arm_ms, radius, damage):
        super().__init__()
        self.game = game
        self.owner = owner
...
class NotebookMissile(pygame.sprite.Sprite):
    ...
    def update(self):
        target = _nearest_enemy_from(self.game, self.pos)
        if target:
...
class MarkerSpray(Weapon):
    ...
    def update(self):
        ...
        if now - self.last_shot <= rate or not self.game.enemies:
            return
        self.last_shot = now
        target = _nearest_enemy_from(self.game, self.owner.pos)
        if not target:
            return
...
class USBBoomerang(Weapon):
    ...
    def update(self):
        ...
        b = USBBoomerangSprite(self.game, self.owner, d, dmg)
...
class StickyNotes(Weapon):
    ...
    def update(self):
        ...
        for _ in range(count):
            ...
            mine = StickyMine(self.game, self.owner, x, y, 450, radius, dmg)
...
class NotebookMissiles(Weapon):
    ...
    def update(self):
        ...
        for i in range(count):
            ...
            m = NotebookMissile(self.game, self.owner, d, dmg)
